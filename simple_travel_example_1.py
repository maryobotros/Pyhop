...
def fly_on_plane(state, a, x, y, z):
...
def travel_by_plane(state, a, x, y, z, ticket_cost):
    if state.cash[a] >= taxi_rate(state.dist[x][y] + ticket_cost):
        return [('buy_plane_ticket', a, ticket_cost), ('travel_by_taxi', a, x, y), ('fly_on_plane', a, y, z)]
    return False
...
print("""
********************************************************************************
Call pyhop.pyhop(state3,[('travel','lex','oxy','new_york')]) with different verbosity levels
********************************************************************************
""")
...
print('- If verbose=2, Pyhop also prints a note at each recursive call:')
pyhop.pyhop(state3, [('travel', 'lex', 'oxy', 'airport')], verbose=2)

print('- If verbose=3, Pyhop also prints the intermediate states:')
pyhop.pyhop(state3, [('travel', 'lex', 'oxy', 'airport')], verbose=3)